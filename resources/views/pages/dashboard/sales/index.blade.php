    ...
    <div class="p-4 sm:ml-64">
        ...
        <section class="p-3 antialiased bg-gray-50 dark:bg-gray-900 sm:p-5">
            <div class="max-w-screen-xl px-4 mx-auto lg:px-12">
                ...
                <div class="flex justify-center top-11">
                    <x-toaster type="success" text="{{ session('success') }}"/>
                </div>
                @endif
                <!-- Start coding here -->
                <div class="relative overflow-hidden bg-white shadow-md dark:bg-gray-800 sm:rounded-lg">
                    <div class="flex flex-col items-center justify-between p-4 space-y-3 md:flex-row md:space-y-0 md:space-x-4">
                        <div class="w-full md:w-1/2">
                            <form action="/admin/dashboard/sales" method="GET" class="flex items-center">
                        ...
                        <div class="flex flex-col items-stretch justify-end flex-shrink-0 w-full space-y-2 md:w-auto md:flex-row md:space-y-0 md:items-center md:space-x-3">
                            <button type="button" id="createSaleModalButton" data-modal-target="createSaleModal" data-modal-toggle="createSaleModal" class="flex items-center justify-center px-4 py-2 text-sm font-medium text-gray-900 rounded-lg bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800">
                            ...
                            <div class="flex items-center w-full space-x-3 md:w-auto">
                                <form action="/admin/dashboard/sales/export">
                                    <button type="submit" class="flex items-center justify-center w-full px-4 py-2 text-sm font-medium text-gray-900 bg-white border border-gray-200 rounded-lg md:w-auto focus:outline-none hover:bg-gray-100 hover:text-primary-700 focus:z-10 focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700" type="button">
                                        <svg class="-ml-1 mr-1.5 w-5 h-5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 20">
                                ...
                                <form action="/admin/dashboard/sales" method="GET" class="flex items-center">
                                    ...
                                    <div id="filterDropdown" class="z-10 hidden w-64 p-3 bg-white rounded-lg shadow dark:bg-gray-700">
                                        <h6 class="mb-3 text-sm font-medium text-gray-900 dark:text-white">Jenis</h6>
                                        <div class="">
                                            <button
                                                name="jenis"
                                                value=""
                                                class="px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-100 focus:outline-none focus:ring focus:border-blue-300"
                                            >
                                                All
                                            </button>
                                            <button
                                        ...
                                        <h6 class="mb-3 text-sm font-medium text-gray-900 dark:text-white">Urutan Nama</h6>
                                        <div class="">
                                            <button
                                                name="urutan"
                                                value=""
                        ...
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                <tr>
                                    ...
                                    <th scope="col" class="px-4 py-4">Nama Pembeli</th>
                                    <th scope="col" class="px-4 py-4">Barang</th>
                            ...
                            <tbody>
                                @foreach ($sales as $sale)
                                    <tr class="border-b dark:border-gray-700">
                                        ...
                                        <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ $sale->user->name }}</th>
                                        <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ $sale->barang }}</th>
                                        <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ $sale->jenis }}</th>
                                        <td class="px-4 py-3">Rp. {{ number_format($sale->harga, 0, ',', '.') }}</td>
                                        <td class="flex items-center justify-end px-4 py-3">
                                            <button id="{{ $sale->id }}-button" data-dropdown-toggle="{{ $sale->id }}" class="inline-flex items-center text-sm font-medium hover:bg-gray-100 dark:hover:bg-gray-700 p-1.5 dark:hover-bg-gray-800 text-center text-gray-500 hover:text-gray-800 rounded-lg focus:outline-none dark:text-gray-400 dark:hover:text-gray-100" type="button">
                                                <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewbox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                                    <path d="M6 10a2 2 0 11-4 0 2 2 0 014 0zM12 10a2 2 0 11-4 0 2 2 0 014 0zM16 12a2 2 0 100-4 2 2 0 000 4z" />
                                                </svg>
                                            </button>
                                            <div id="{{ $sale->id }}" class="z-10 hidden bg-white divide-y divide-gray-100 rounded shadow w-44 dark:bg-gray-700 dark:divide-gray-600">
                                                <ul class="py-1 text-sm" aria-labelledby="apple-imac-27-dropdown-button">
                                                    <li>
                                                        <button type="button" data-modal-target="updateSaleModal" data-modal-toggle="updateSaleModal" data-id={{ $sale->id }} data-user="{{ $sale->user->name }}" data-barang="{{ $sale->barang }}" data-harga={{ $sale->harga }} class="flex items-center w-full px-4 py-2 text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white dark:text-gray-200">
                                                    ...
                                                    <li>
                                                        <button type="button" data-modal-target="deleteModal" data-modal-toggle="deleteModal" data-id={{ $sale->id }} class="flex items-center w-full px-4 py-2 text-red-500 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-red-400">
        ...
        <!-- End block -->
        <!-- Create modal -->
        <x-sales.create></x-sales.create>
        <!-- Update modal -->
        <x-sales.update></x-sales.update>
        <!-- Delete modal -->
        <x-sales.delete></x-sales.delete>
        <!-- Read modal -->
